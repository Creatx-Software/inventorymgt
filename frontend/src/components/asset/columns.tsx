...
export function commonAssetColumns<T extends AssetCommon>(): ColumnDef<T, any>[] {
  return [
    ...
    { accessorKey: 'vendor_name', header: 'Vendor', size: 150, cell: (i) => i.getValue() || <span className="text-slate-300">—</span> },
    { accessorKey: 'model', header: 'Model', size: 180, cell: (i) => i.getValue() || <span className="text-slate-300">—</span> },
    { accessorKey: 'status_name', header: 'Status', size: 120,
      cell: (i) => <StatusBadge name={i.row.original.status_name} color={i.row.original.status_color} /> },
    { accessorKey: 'employee_name', header: 'Assigned To', size: 200,
      cell: (i) => {
    ...
    { accessorKey: 'location_name', header: 'Location', size: 180, cell: (i) => i.getValue() || <span className="text-slate-300">—</span> },
    { accessorKey: 'department_name', header: 'Department', size: 150, cell: (i) => i.getValue() || <span className="text-slate-300">—</span> },
